	...
	*/
	...
	if ($file)
	{
		while (!feof($file))
		{
			...
			$result = pg_query($connection, "INSERT INTO nodes (id, tags, geom) VALUES ('".$data[0]."', '".$tags."', GeometryFromText('POINT ( ".$data[2]." ".$data[3]." )', 4326 ))");
		}
	}
	fclose($file);
	echo "Finished nodes...\n";

	$file = fopen("olm-ways.csv", "r");
	if ($file)
	{
		while (!feof($file))
		{
			...
			$result = pg_query($connection, "INSERT INTO ways (id, tags, geom) VALUES ('".$data[0]."', '".$tags."', GeometryFromText('POINT ( ".$data[2]." ".$data[3]." )', 4326 ))");
		}
	}
	fclose($file);
	echo "Finished ways...\n";
