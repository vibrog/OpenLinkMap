<?php
	...
	*/
	...
	function getDetails($db, $id, $type, $langs, $offset)
	{
		...
		if ($response)
		{
			if ($format == "text")
				...
				echo xmlMoredetailsOut($response[0], $langs, $offset, $id, $type, $db);
		...
		if ($response)
		{
			...
			if (substr($response['image'], 0, 28) == "http://upload.wikimedia.org/")
			{
				...
				$output .= "<table>\n";
					...
					$output .= "<tr><td><img id=\"moreImage\" onclick=\"showFullscreen('".$response['image']."');\" title=\"".$translations['captions']['fullscreen']."\" src=\"".$response['image']."\" /></a></td></tr>\n";
			...
			elseif (getWikipediaImage($wikipedia[1]))
			{
				...
				$output .= "<table>\n";
					$output .= "<tr><td><strong>".$translations['captions']['image']."</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i id=\"moreWikipediaFull\"><a target=\"_blank\" href=\"http://commons.wikimedia.org/w/index.php?title=Special%3ASearch&search=".$name."\">".$translations['captions']['moreimages']."</a></i></td></tr>\n";
					$output .= "<tr><td><img id=\"moreImage\" onclick=\"showFullscreen('".$image."');\" title=\"".$translations['captions']['fullscreen']."\" src=\"".$image."\" /></a></td></tr>\n";
			...
			if (substr($response['panorama'], 0, 28) == "http://upload.wikimedia.org/")
			{
				...
				$output .= "<table>\n";
					...
					$output .= "<tr><td><img id=\"moreImage\" onclick=\"showPanorama('".$response['image']."');\" title=\"".$translations['captions']['fullscreen']."\" src=\"".$response['panorama']."\" /></a></td></tr>\n";
					$output .= "<tr><td><a target=\"_blank\" href=\"http://commons.wikimedia.org/wiki/File:".$attribution[7]."\">".$translations['captions']['copyrightandbig']."</a></td></tr>\n";
				$output .= "</table>\n";
				$output .= "</div>\n";
			}
	...
	function xmlMoreDetailsOut($response, $langs = "en", $offset = 0, $id, $type, $db)
	{
		if ($response)
		{
			$output = xmlStart("moredetails database=\"".$db."\" id=\"".$id."\" type=\"".olm2osm($type)."\" olmtype=\"".$type."\"");
